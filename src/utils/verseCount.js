...
const DEFAULT_VERSE_COUNT = 28; // fallback average

/**
 * Returns the number of verses in a given book and chapter.
 * @param {string} bookName - Full book name (e.g. "Genesis")
 * @param {number} chapter - 1-based chapter number
 * @returns {number} verse count
 */
export function getVerseCount(bookName, chapter) {
  const chapters = VERSE_COUNTS_BY_BOOK[bookName];
  if (!chapters) return DEFAULT_VERSE_COUNT;
  const count = chapters[chapter - 1];
  return count ?? DEFAULT_VERSE_COUNT;
}